...
def generate_report_number() -> str:
    return f"RPT-{datetime.utcnow().strftime('%Y%m%d')}-{secrets.token_hex(4).upper()}"
...
<div class="footer">
    ...
    <p>&copy; {datetime.utcnow().year} APEX Housing. All rights reserved.</p>
</div>
...
class BookingReportService:
    ...
    async def sign_report(self, report_id: UUID, user_id: UUID, role: str, ip_address: str = None, signature_data: str = None) -> BookingReport:
        ...
        if role == "LANDLORD":
            ...
            report.landlord_signed_at = datetime.utcnow()
